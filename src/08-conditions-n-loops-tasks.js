 ...
 *
 */
function doRectanglesOverlap(rect11, rect22) {
  const r1 = rect11;
  ...
  const isIntersecting = !(r2.left > r1.right
    || r2.right < r1.left
    || r2.top > r1.bottom
    || r2.bottom < r1.top);
  const isContained = isInside(r1, r2) || isInside(r2, r1);
...
function isBracketsBalanced(str) {
  const brackets = [['(', ')'], ['[', ']'], ['{', '}'], ['<', '>']];
  const open = new Set(brackets.map((pair) => pair[0]));
  const close = new Set(brackets.map((pair) => pair[1]));
  const relevant = brackets.reduce((acc, [open1, close1]) => ({ ...acc, [close1]: open1 }), {});
